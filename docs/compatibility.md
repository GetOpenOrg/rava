...
| `Object.clone`（浅拷贝 / Cloneable 契约 / 数组 clone） | 近似等价 | TestObjectClone：浅拷贝、非 Cloneable 抛 CloneNotSupportedException、数组 clone 五形态**已实测等价**；**偏差**：类自身声明的 clone 覆盖体不执行（调用点直接浅拷贝），深拷贝覆盖与经基类视角的虚分派不生效 | C-1（待宏修复授权） |
