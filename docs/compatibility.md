...
| JCA（`MessageDigest` / `Cipher` / `SecureRandom`） | **语义等价**（2026-09-25，K-JCA） | 摘要与分组密码实现来自 JDK provider （SUN / SunJCE）实现类的**翻译字节码**，服务表从 provider 注册字节码抽取；入选服务 = engine 类在调用链上 × 算法名是用户字符串常量。**偏差**：①运行期拼接、字节码不可见的算法名不入选 → `NoSuchAlgorithmException`（与未知算法同形态）；②算法别名（OID、`SHA256` 等）未登记，只认规范名（大小写不敏感）；③服务属性（SupportedModes/Paddings/KeyFormats）未登记：模式/填充由实现类自身校验，错误组合的异常在 `init` 而非 `getInstance` 抛出；④JCE 策略恒 unlimited（JDK 9+ 默认）；⑤`SecureRandom` 取操作系统熵源（`/dev/urandom`，NativePRNG 同语义），`SecureRandom.getInstance(算法)` 显式选算法未实现（精确存根）；⑥`Security.addProvider` / 第三方 provider 不支持 | K-JCA |
| `Class.forName`（含三参数形式） | **条件等价**（2026-09-26） | 可加载类 = 生成闭包内的类（含用户类）+ 数组名；未知名 → `ClassNotFoundException`（消息同 JDK）。**偏差**：①原生单镜像无类加载器层级——`loader=null` 也能取到用户类（JDK 为 CNFE）；`VM.latestUserDefinedLoader` 恒 null；②调用链未触达的 JDK 类不在镜像内 → CNFE（JDK 可加载）；③`initialize=true` 不立即执行 `<clinit>`，首次静态访问时初始化 | S-66 |
| `Class.getFields`（public 字段含继承 / 接口常量） | **条件等价**（2026-09-26） | 字段集合与 JDK 一致（本类 public → 接口常量 → 超类递归，去重）。**偏差**：无直接超接口表，接口常量按超类型闭包表顺序枚举，与 JDK「直接超接口递归」顺序可能不同 | S-66 |
