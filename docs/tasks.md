...
| **JUnit pilot M1+M2（libpilot 代理在途）** | `/tmp/wt-libpilot` 基 a63df7c | hamcrest crate（jar 输入+lib 发射两机制）+ junit Assert 子集；golden 对账+闭包量级对标。任务书=方案附录定稿；jar 资产/fetch.sh 路径已备 |
