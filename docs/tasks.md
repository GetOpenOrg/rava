...
| **G-3 下一层三件包（next3 已合入 `6858233`，主目标 0 绿但全推进）** | 回归 8/8、双种子归零 | 族 B（DateTimeFormat）：手写边界重载判定按描述符精确化（`fn add` 前缀子串把 `add_obj_arr_obj_i` 误判成 `add_obj` 手写）——E0308 根除；**identityHashCode native 已手写（`695377a`），现推至 CalendarDataUtility locale 数据链 stub**。族 A（FilesApi/ZonedDateTime）：边界接口回调边机制**休眠态落地**（激活实验证 stub 消除，但唤醒 ObjectStreamClass 休眠序列化暴露 blocks.py unify 两族编译缺口[三目合并退化 E0599 + 双局部声明载体 E0308]——当时禁改域）+ **BFS 双种子确定性修复**（基线既有 set 无序迭代，闭包 ±1 漂移）。**unify4 代理在途**（/tmp/wt-unify4 基 695377a）：段 1 修两族→段 2 翻激活开关→段 3 两例冲绿 + VT 构造器顺查 |
...
| **JUnit→Rust 依赖 crate pilot（M1/M2 就绪待派发）** | `2026-09-23-junit-crate-pilot.md`（另一会话交付的 P0 落地设计） | 方案成熟：六前置/五里程碑/任务书草稿齐（附录）；域核查与在途零重叠（callchain 无人占用、project_writer 无在途改动）。**jar 资产已拉齐**（pilot-deps/target/pilot-libs：hamcrest-3.0/hamcrest-core-3.0/junit-4.13.2，fetch.sh 的 dep_scan 路径已修为相对解析）。M1=hamcrest 先行成 crate（jar 输入+lib 发射两机制）、M2=junit Assert 子集——纯增量工程，等本机修复代理位（unify4/jdk25-l1 在途）空出即派发。M3 反射 L3 挂 A-4/TypeIR 合流、M4 线程边界挂线程模型决策、M5 终态挂重写 R9 |
