...
| K-JCA | **JCA 服务注册**：Cipher / MessageDigest / SecureRandom | 🔄 验证中（jcaB3，17 例） | `docs/plans/2026-09-25-jca-service-registry.md`。服务表 + 种子 + 放行 + 手写边界已完成；验证链揭出并修复 10 余项通用缺口（见归档 2026-09-25/26 批次）。进展：DES 过 getInstance/init 至 doFinal、MD5/SHA-1/SHA-256 摘要输出与 JVM 一致、SecureRandom.nextBytes 正确。**用户 JDK21 回报后补**：CryptoAlgorithmConstraints.permits（新版 JDK21 更新 / JDK25 的 jdk.crypto.disabledAlgorithms 检查，默认禁用集为空 → true，`1e9da59`+`1425bec` 重载名修正） |
...
| N12 | JDK25 TestCompletableFuture 运行 4 分钟（JDK21 0.3s） | 🔄 已实现（`a8c027e`），待验证（本机 fix2j25 + 用户 JDK25） | 根因：协作调度下 Unsafe.park 立即返回，JDK25 FJP `awaitWork` 以 currentTimeMillis + park(true, deadline) 等 keepAlive，worker 空转至真实截止（60s × 4）。方案：**虚拟时钟**——限时 park / wait(ms) 在泵内无线程可推进时只能超时结束，时钟偏移前移至截止（nanoTime / currentTimeMillis 叠加单调偏移），sleep 时长同样前移兑现；无限期 park 不跳。e2e：TestVirtualClockPark |
