...
| **G-3 三重复用槽（WIP 接管中）** | 分支 `fix/g3-triple-slot` @ `7f94f65`（另一台机器推送的 WIP 归档，机器已退役主会话接管） | `_forms_alignable` 分型框架：可对齐（同型/引用子类型 .into()）并入提升绑定；不可对齐（基本 vs 引用，try 暂存/监视对象/catch 形参同槽）=同槽两个 JVM 变量独立绑定。**静态验收全过**：ZonedDateTime/FilesApi cargo check 零错误（前任"未过"系中断前陈旧态）、DateTimeFormat 双种子归零。待 release 运行批（错峰等 cf-anewarray）+ OptionalFull 复核 + TestSuppressed 回归 |
| ~~anewarray 组件类型 E0283（cf-anewarray 代理）~~ | **✅ 合入（3 提交，merge 见 main；代理自验回归 10/10、双种子归零、审计持平）** | 根因=sim/arrays.py 对泛型组件把 Object 替换成 `_`（0e6ff4f 手法）——varargs 数组只经 Object 边界流转无推断锚。修法=**anewarray 发射即终态**（删 `_` 分支，直接发射 `C<Object>`/载体接口形态；异实参存储由 aastore 既有 Object 边界检查兜住）+ CF 运行时补链四件（SharedThreadContainer/FJPAccess/getIntOpaque/AtomicInteger.\<init\>）+ availableProcessors=1（单线程协作档位，等价 -XX:ActiveProcessorCount=1）。**TestCompletableFuture 编译过、run 推进 5 卡点后停在 `native: VarHandle.set`** |
| **VarHandle 引用族访问模式（TestCompletableFuture 下一层，run 层新发现）** | cf-anewarray 代理交付披露 | CompletableFuture 的 RESULT/STACK/NEXT 无锁完成栈全依赖引用族 VarHandle（set/compareAndSet/getAcquire）——现有 `var_handle_impl.rs` 只覆盖 int/long/boolean 族。修法方向：`_field_offset` 加 `VarHandleReferences$FieldInstance` 臂 + 引用字段 CAS 协议 + 可能宏侧引用 cell 臂（`__unsafe_ref_cell`? 与 `__unsafe_int/long_cell` 同型）。此前编译期即失败，run 层不可见——深层缺口首次暴露 |
