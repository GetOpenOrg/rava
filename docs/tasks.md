...
| ~~ice 修复①+②（default 父类链 + unify_pair）~~ | **✅ 完成（`33a63e9`+`15565bf`，主会话 5/5 复核）** | **3 例全绿**（HashSetOps/MapIteration/TreeMapSet）：default 注入覆盖判定沿父类链累积（+21 行，_anc_ifaces 防线不动）+ unify_pair 不灭真值（具体臂经 Object 边界重建，86bf63a 同型；null 臂合法路径未误伤）。**顺带恢复 CompletableFuture.postComplete 三元语义**（基线隐性灭 null，因编译未过从未暴露）。调查表修复③④仍在队（ListIterator 断链→另一机、string null 守卫） |
