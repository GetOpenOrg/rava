...
**基线（2026-09-23 服务器 `--failed` 轮 @ b0f5460 + 主会话三修）**：21 例跑 6 出列（CollectorsMore/Pecs/ArrayBounds/AutoboxEdge/Atomics/SequencedCollections——近期合并全部兑现）→ 保留 15。**b0f5460 是本地 main 祖先、落后 6 提交**：K-6b（TestStreamNumeric 全绿）+ param-rebind 双件（TestBigInteger 全绿）已在本地，pull 后 15→13。主会话当日三修再 -3：TestUnnamedVar（javac 预览门 `57b0dac`）/ TestEnumSetMap（反射 init 钩子+元数据表用户域 `2741344`）/ TestStringEdge E0432（companion 依赖闭包门槛 `aca9acc`，残余 internSame 2 行与 TestStringCompare 同根因归 output3）→ **下轮 pull+回归后清单预期 15→10**：unify3 域 2（StringNewMethods 编译错+StreamMore CCE 同族）、output3 域 2（StringCompare/ArrayCovariance）、g3 域 3（DateTimeFormat/ZonedDateTime/FilesApi）、CompletableFuture（anewarray `_` 已诊断）、RecordPattern（describe stub G-10 邻域）、VirtualThread（ContinuationSupport stub）。新架构落地：**类初始化钩子表**（反射按名路径强制初始化，JVM 语义对齐）+ **元数据四表扫描域=运行时树+用户树**（Class.getSuperclass 等反射层次对用户类生效）+ **companion 依赖闭包门槛**（手写 impl 硬依赖语料生成类时声明以完整性为前提）。
