...
| **TestVirtualThread 终态定性（2026-09-24 试探裁定）** | **线程模型已定方案 A（任务面板 #24）**；09-24 晚实测卡点=`stub: java/lang/VirtualThread.<init>:(Ljava/util/concurrent/Executor;Ljava/lang/String;ILjava/lang/Runnable;)V` | executors-fix 修三错后解除态**编译过**（63+1514）；主会话试探批实证运行推进到 `ContinuationScope.<init>` 构造器 stub——下一层=**Continuation 机制本体**（yield/unmount/mount 的 VM 深耦合，需协作调度等价承载的架构工作）。**vm_boundary 维持截断**（试探行已还原），VT 挂 roadmap §四-2 线程模型终态决策——非 codegen 缺口，不进 169 冲刺。ContinuationSupport 恒真/park 泵/executors 三错修复已沉淀为该决策的前置资产 |
