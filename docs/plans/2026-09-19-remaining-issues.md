...
### S-18 经接口分派的方法实现体未进 BFS 调用链 【已修复，S-18 轮】

> **已修复**（fix/s18-iface-dispatch-bfs，基于 a20a31f）：根因不是「传播只查直接 implements」（`_supertypes` 传递闭包自 c6771aa 拆分起就在），而是**传播与解析都只在「常量池类自身声明的方法」上工作**：`invokeinterface Stream.sequential` 的常量池类 `Stream` 未声明该方法（声明在父接口 `BaseStream`，实现体在抽象类 `AbstractPipeline`），`_propagate_virtual_targets` 里 `decl = ci.methods` 精确查找得 None 直接 continue；`_enqueue_declaring_method` 的接口闭包查找又把 abstract 声明过滤掉（`not m.is_abstract`）→ 84 条 unresolved + 实现体永不入队。修复：(1) 声明槽位解析 `_resolve_virtual_slot`（JVMS §5.4.3.3：本类→父类链→接口闭包，abstract 同为有效槽位），传播时常量池类未声明则沿层次解析后再走既有「接口→传递实现类」与 RTA 收录；(2) 接口闭包查找接受 abstract 声明为已解析（不入队、但不算 unresolved）。配套：`_find_virtual_in` 废除「重载 mangle 状态差异中止追踪」守卫（子类因实现接口方法新增同名重载时误判本类新槽位 → 祖先槽位无人覆盖 → 虚分派落 trait default stub），覆盖条目 wrapper 名（本类重载态）与 vtable 槽位名（声明者态）经 `vtable_name` 属性解耦（types/attrs/class_writer/inherited_gen/vtable_util + 宏 virtual_dispatch/wrapper，与继承成员的 K-6 机制统一）；`method_name_is_mangled` 单一权威化为纯接收者态判定；`_mangle_if_overloaded` 按接收者/接口载体分视角；`return (S) this` 经 Object 边界身份重建（returns.py）。**实测**：TestStreamAdvanced 输出与 expected 逐字一致、退出码 0（纯 main a20a31f 命中 sequential stub panic）；19 个红线测试全 PASS；unresolved 84→0，sig-poly-native 保持 22（与 main 相同），root-inherited 2794→2776；闭包 1354→1362 类 / java_method 26677→26837；覆盖条目 vtable_name 0→27、继承成员槽位改名 67→97。**未回归的既有失败**（main 上同形态，另案）：TestStreamCollectors（JavaLangAccess.join stub）、TestStringBuilderOps（DoubleToDecimal.appendTo stub，P-3 手写层补全）、TestStreamNumeric（Object 接收者上 exactOutputSizeIfKnown 成员解析 E0599）、TestVarContext（DataHolder E0433）。
...
| 5 | 栈帧未填充（`fillInStackTrace`/`getStackTrace` 空） | TestCustomException | **✅ 已修（`2e35b9e`）**：`fillInStackTrace` 经 `std::backtrace::force_capture`（零新依赖）捕获真实 Rust 栈，`initStackTraceElements` 展开为元素数组；帧头对齐 JVM（探针首四帧 level3/level2/level1/main 与 JDK 21 同名同序，构造器帧隐藏）；档位=帧数真实、内容近似（已声明边界）——**S-19 六件全部收官** |
