...
> ① e2e **65/65 全绿**（路线图 [2026-09-19-remaining-issues.md](2026-09-19-remaining-issues.md) 走完）；
> ② Python 拆分方案 [2026-09-20-source-file-split-plan.md](2026-09-20-source-file-split-plan.md) **窗口 0–3 全部完成**（§五 终态指标达成）；
> ③ 批次 5 收尾项中 **P-1（JDK 字面量清零）与 G-4（生成确定性）先行完成**——前者决定哪些表"不迁移"，后者是 golden diff 可比性的前提。
>
