...
public class StreamToUnmodifiableCollections {
    public static void main(String[] args) {
        ...
        // toUnmodifiableList preserves encounter order — deterministic
        List<String> immutableList = list.stream()
                .collect(Collectors.toUnmodifiableList());
        immutableList.forEach(System.out::println);

        // toUnmodifiableMap has undefined iteration order — sort by key before printing
        Map<String, Integer> wordsLengths = list.stream()
                .collect(Collectors.toUnmodifiableMap(Function.identity(), String::length));
        new TreeMap<>(wordsLengths)
                .forEach((word, length) -> System.out.println(word + " -> " + length + " letters"));
    }
}
