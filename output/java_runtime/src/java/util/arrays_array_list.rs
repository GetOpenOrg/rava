...
impl<E: Clone + Default + 'static> Arrays_ArrayList<E> {
    ...
    pub fn new(mut array: Rc<RefCell<Vec<E>>>) -> Result<Self> {
        ...
        this.a.set(Clone::clone(&(_t0).downcast::<Rc<RefCell<Vec<E>>>>()));
        Ok(this)
    }
    ...
    pub fn forEach(&self, action: Consumer<E>) -> Result<()> {
    ...
    pub fn sort(&self, c: Comparator<E>) -> Result<()> {
