...
java_rta_macros::java_class! {
    ...
    impl ArrayList_Itr {
        #[java_method(name = "<init>", descriptor = "(Ljava/util/ArrayList;)V", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, method_parameters = ":4112")]
        ...
        pub fn hasNext(&self) -> Result<bool> {
            let this = self;
            Ok(this.__get_cursor() != this.__get_this_0().__get_size())
        }
        ...
        pub fn next(&self) -> Result<Object> {
            ...
            if i >= this.__get_this_0().__get_size() {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            let mut elementData = this.__get_this_0().__get_elementData();
            if i >= (elementData.borrow().len() as i32) {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            this.__set_cursor((i).wrapping_add(1i32));
            this.__set_lastRet(i);
            Ok(Clone::clone(&elementData.borrow()[i as usize]))
        }
        ...
        pub fn remove(&self) -> Result<()> {
            let this = self;
            if (this.__get_lastRet()<0) {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            this.checkForComodification()?;
            let _t0 = this.__get_this_0().remove_i(this.__get_lastRet())?;
            this.__set_cursor(this.__get_lastRet());
            this.__set_lastRet(-1i32);
            this.__set_expectedModCount(this.__get_this_0().__get_modCount());
            Ok(())
        }
        ...
        pub fn checkForComodification(&self) -> Result<()> {
            let this = self;
            if this.__get_this_0().__get_modCount() != this.__get_expectedModCount() {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            Ok(())
        }
