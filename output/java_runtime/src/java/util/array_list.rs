...
impl<E: Clone + Default + 'static> From<ArrayList<E>> for AbstractList<E> {
    fn from(v: ArrayList<E>) -> AbstractList<E> { v.__into_super() }
}

impl<E: Clone + Default + 'static> From<ArrayList<E>> for AbstractCollection<E> {
    fn from(v: ArrayList<E>) -> AbstractCollection<E> { v.__into_super().__into_super() }
}
...
java_rta_macros::java_class! {
    ...
    pub struct ArrayList<E: Clone + Default + 'static> {
        ...
        pub elementData: Rc<RefCell<Vec<Object>>>,
    ...
    impl<E> ArrayList<E> {
        ...
        pub fn new_i(mut initialCapacity: i32) -> Result<Self> {
            ...
            if (initialCapacity>0) {
                let mut _arr0: Rc<RefCell<Vec<Object>>> = Rc::new(RefCell::new(vec![Default::default(); initialCapacity as usize]));
                this.__set_elementData(Clone::clone(&_arr0));
            } else {
                if (initialCapacity==0) {
                    this.__set_elementData(Clone::clone(&ArrayList::<Object>::EMPTY_ELEMENTDATA()));
                } else {
                    let _t0 = StringBuilder::new()?.append_str(Clone::clone(&String::from("Illegal Capacity: ")))?;
        ...
        pub fn new() -> Result<Self> {
            let mut this = Self::default();
            this = Self::__new_with_super(AbstractList::new()?);
            this.__set_elementData(Clone::clone(&ArrayList::<Object>::DEFAULTCAPACITY_EMPTY_ELEMENTDATA()));
            Ok(this)
        }
        ...
        pub fn new_coll(mut c: Object) -> Result<Self> {
            let mut this = Self::default();
            this = Self::__new_with_super(AbstractList::new()?);
            let _vdispatch0: Rc<RefCell<Vec<Object>>> = if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList_RandomAccessSubList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<LinkedList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList_SubList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayList_SubList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractSequentialList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Arrays_ArrayList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashMap_EntrySet>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashMap_KeySet>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<TreeMap_EntrySet>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<LinkedHashSet<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayDeque<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractSet<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashSet<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractCollection<Object>>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Object>() { _d.toArray()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Object>() { _d.toArray()? } else if let Some(__f) = c.0.as_any().downcast_ref::<std::rc::Rc<dyn Fn() -> crate::error::Result<Rc<RefCell<Vec<Object>>>>>>() { (__f)()? } else { Default::default() };
            let mut a: Rc<RefCell<Vec<Object>>> = _vdispatch0;
            this.__set_size((a.borrow().len() as i32));
            if ((a.borrow().len() as i32)!=0) {
                let _vdispatch1: Object = if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList_RandomAccessSubList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<LinkedList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList_SubList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayList_SubList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractSequentialList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Arrays_ArrayList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashMap_EntrySet>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashMap_KeySet>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<TreeMap_EntrySet>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<LinkedHashSet<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractList<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<ArrayDeque<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractSet<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<HashSet<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<AbstractCollection<Object>>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Object>() { _d.getClass()? } else if let Some(_d) = c.0.as_any().downcast_ref::<Object>() { _d.getClass()? } else if let Some(__f) = c.0.as_any().downcast_ref::<std::rc::Rc<dyn Fn() -> crate::error::Result<Object>>>() { (__f)()? } else { Default::default() };
                if _vdispatch1 == Object::default() {
                    this.__set_elementData(Clone::clone(&a));
                } else {
                    let _t2: Rc<RefCell<Vec<Object>>> = Arrays::copyOf_arr_obj_i_class(Clone::clone(&a), this.__get_size(), Default::default())?;
                    this.__set_elementData(Clone::clone(&_t2));
                }
            } else {
                this.__set_elementData(Clone::clone(&ArrayList::<Object>::EMPTY_ELEMENTDATA()));
            }
        ...
        #[java_method(name = "grow", descriptor = "(I)[Ljava/lang/Object;", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        // java: grow(I)[Ljava/lang/Object;
        pub fn grow_i(&self, mut minCapacity: i32) -> Result<Rc<RefCell<Vec<Object>>>> {
            let this = self;
            let mut oldCapacity = (this.__get_elementData().borrow().len() as i32);
            if Object::from_any(this.__get_elementData().clone()) != Object::from_any(ArrayList::<Object>::DEFAULTCAPACITY_EMPTY_ELEMENTDATA().clone()) {
                ...
                let _t1: Rc<RefCell<Vec<Object>>> = Arrays::copyOf_arr_obj_i(Clone::clone(&this.__get_elementData()), newCapacity)?;
                this.__set_elementData(Clone::clone(&_t1));
                return Ok(_t1);
            }
            ...
            this.__set_elementData(Clone::clone(&_arr1));
            Ok(_arr1)
        }

        #[java_method(name = "grow", descriptor = "()[Ljava/lang/Object;", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        // java: grow()[Ljava/lang/Object;
        pub fn grow(&self) -> Result<Rc<RefCell<Vec<Object>>>> {
            let this = self;
            let _t0 = this.grow_i((this.__get_size()).wrapping_add(1i32))?;
            Ok(_t0)
        }
        ...
        #[java_method(name = "indexOfRange", descriptor = "(Ljava/lang/Object;II)I", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        #[java_method(name = "lastIndexOfRange", descriptor = "(Ljava/lang/Object;II)I", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        pub fn toArray(&self) -> Result<Rc<RefCell<Vec<Object>>>> {
            let this = self;
            let _t0: Rc<RefCell<Vec<Object>>> = Arrays::copyOf_arr_obj_i(Clone::clone(&this.__get_elementData()), this.__get_size())?;
            Ok(_t0)
        }
        ...
        pub fn toArray_arr_obj(&self, mut a: Rc<RefCell<Vec<Object>>>) -> Result<Rc<RefCell<Vec<Object>>>> {
            ...
            if (a.borrow().len() as i32) < this.__get_size() {
                ...
                let _t1: Rc<RefCell<Vec<Object>>> = Arrays::copyOf_arr_obj_i_class(Clone::clone(&this.__get_elementData()), this.__get_size(), Clone::clone(&_t0))?;
                return Ok(_t1);
            }
            System::arraycopy(Object::from_any(this.__get_elementData().clone()), 0i32, Object::from_any(a.clone()), 0i32, this.__get_size())?;
            if (a.borrow().len() as i32) > this.__get_size() {
        ...
        #[java_method(name = "elementAt", descriptor = "([Ljava/lang/Object;I)Ljava/lang/Object;", access = "package", modifiers = "static", is_static    = true, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "<E:Ljava/lang/Object;>([Ljava/lang/Object;I)TE;")]
        ...
        pub fn add_obj(&self, mut e: E) -> Result<bool> {
            ...
            this.add_obj_arr_obj_i(Clone::clone(&e), Clone::clone(&this.__get_elementData()), this.__get_size())?;
            Ok((1i32 != 0i32))
        }
        ...
        pub fn remove_i(&self, mut index: i32) -> Result<E> {
            let this = self;
            let _t0: i32 = Objects::checkIndex_i_i(index, this.__get_size())?;
            let mut es = this.__get_elementData();
            let mut oldValue = Clone::clone(&es.borrow()[index as usize]);
            this.fastRemove(Clone::clone(&es), index)?;
            Ok(oldValue)
        }
        ...
        #[java_method(name = "equalsRange", descriptor = "(Ljava/util/List;II)Z", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "(Ljava/util/List<*>;II)Z")]
        ...
        #[java_method(name = "equalsArrayList", descriptor = "(Ljava/util/ArrayList;)Z", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "(Ljava/util/ArrayList<*>;)Z")]
        ...
        #[java_method(name = "checkForComodification", descriptor = "(I)V", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        #[java_method(name = "hashCodeRange", descriptor = "(II)I", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        #[java_method(name = "fastRemove", descriptor = "([Ljava/lang/Object;I)V", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        pub fn fastRemove(&self, mut es: Rc<RefCell<Vec<Object>>>, mut i: i32) -> Result<()> {
            let this = self;
            this.__set_modCount((this.__get_modCount()).wrapping_add(1i32));
            let mut newSize = (this.__get_size()).wrapping_sub(1i32);
            if (this.__get_size()).wrapping_sub(1i32) > i {
                System::arraycopy(Object::from_any(es.clone()), (i).wrapping_add(1i32), Object::from_any(es.clone()), i, (newSize).wrapping_sub(i))?;
            }
            this.__set_size(newSize);
            es.borrow_mut()[newSize as usize] = Clone::clone(&Object::default());
            Ok(())
        }
        ...
        #[java_method(name = "shiftTailOverGap", descriptor = "([Ljava/lang/Object;II)V", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        #[java_method(name = "outOfBoundsMsg", descriptor = "(II)Ljava/lang/String;", access = "private", modifiers = "static", is_static    = true, is_native    = false, is_abstract  = false, is_synthetic = false)]
        ...
        #[java_method(name = "batchRemove", descriptor = "(Ljava/util/Collection;ZII)Z", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "(Ljava/util/Collection<*>;ZII)Z")]
        ...
        pub fn listIterator(&self) -> Result<Object> {
            let this = self;
            Ok(Object::from_any(ArrayList_ListItr::new(Clone::clone(this), 0i32)?.clone()))
        }
        ...
        pub fn iterator(&self) -> Result<Object> {
            let this = self;
            Ok(Object::from_any(ArrayList_Itr::new(Clone::clone(this))?.clone()))
        }
        ...
        pub fn subList(&self, mut fromIndex: i32, mut toIndex: i32) -> Result<Object> {
            let this = self;
            ArrayList::<Object>::subListRangeCheck(fromIndex, toIndex, this.__get_size())?;
            Ok(Object::from_any(ArrayList_SubList::<Object>::new_arrayl_i_i(Clone::clone(this), fromIndex, toIndex)?.clone()))
        }
        ...
        pub fn forEach(&self, mut action: Object) -> Result<()> {
            ...
            let mut expectedModCount = this.__get_modCount();
            let mut es = this.__get_elementData();
            let mut size = this.__get_size();
            let mut i: i32 = 0i32;
            loop {
                if this.__get_modCount() != expectedModCount { break; }
                if i < size {
                    let _t1: Object = ArrayList::<Object>::elementAt(Clone::clone(&es), i)?;
                    if let Some(_d) = action.0.as_any().downcast_ref::<WhileOps_UnorderedWhileSpliterator_OfRef_Dropping<Object>>() { _d.accept(Clone::clone(&_t1))?; } else if let Some(_d) = action.0.as_any().downcast_ref::<WhileOps_UnorderedWhileSpliterator_OfRef_Taking<Object>>() { _d.accept(Clone::clone(&_t1))?; } else if let Some(_d) = action.0.as_any().downcast_ref::<WhileOps_UnorderedWhileSpliterator_OfRef<Object>>() { _d.accept(Clone::clone(&_t1))?; } else if let Some(_d) = action.0.as_any().downcast_ref::<Object>() { _d.accept(Clone::clone(&_t1))?; } else if let Some(__f) = action.0.as_any().downcast_ref::<std::rc::Rc<dyn Fn(Object) -> crate::error::Result<()>>>() { (__f)(Clone::clone(&_t1))?; }
                    i = i.wrapping_add(1i32);
                    continue;
                }
                break;
            }
            if this.__get_modCount() != expectedModCount {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            Ok(())
        }
        ...
        #[java_method(name = "removeIf", descriptor = "(Ljava/util/function/Predicate;II)Z", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "(Ljava/util/function/Predicate<-TE;>;II)Z")]
        ...
        #[java_method(name = "replaceAllRange", descriptor = "(Ljava/util/function/UnaryOperator;II)V", access = "private", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, generic_signature = "(Ljava/util/function/UnaryOperator<TE;>;II)V")]
        ...
        pub fn sort(&self, mut c: Object) -> Result<()> {
            let this = self;
            let mut expectedModCount = this.__get_modCount();
            Arrays::sort_arr_obj_i_i_compar(Clone::clone(&this.__get_elementData()), 0i32, this.__get_size(), Clone::clone(&c))?;
            if this.__get_modCount() != expectedModCount {
                return Err(JvmError::Custom("athrow".to_owned()));
            }
            this.__set_modCount((this.__get_modCount()).wrapping_add(1i32));
