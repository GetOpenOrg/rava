...
//
// 字段注入：@field 指令让 emitter 在生成的 InternalLock struct 中追加额外字段。
// 该字段不受 Field<T> 包装，也不对应任何 Java 字段——仅供本 native 实现使用。
...
pub fn unlock(_this: &InternalLock) -> Result<()> {
    // 弹出栈顶 guard → drop → 释放锁
