...
java_rta_macros::java_class! {
    ...
    #[inner_classes     = "TestStaticNested$LinkedList:TestStaticNested:LinkedList:8;TestStaticNested$Node:TestStaticNested:Node:8"]

    // ── 宏展开输入 ──────────────────────────────────────────────
    #[is_interface      = false]
    #[all_supertypes    = "TestStaticNested;java/lang/Object"]

    pub struct TestStaticNested;

    impl TestStaticNested {
        ...
        pub fn main() -> Result<()> {
            let mut list = TestStaticNested_LinkedList::new()?;
            list.add(1i32)?;
            list.add(2i32)?;
            list.add(3i32)?;
            list.print()?;
