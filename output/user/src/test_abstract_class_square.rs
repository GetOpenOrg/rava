...
impl From<TestAbstractClass_Square> for TestAbstractClass_Shape {
    fn from(v: TestAbstractClass_Square) -> TestAbstractClass_Shape { v.__into_super() }
}

java_rta_macros::java_class! {
    // ── 字节码元数据 ──────────────────────────────────────────────
    #[binary_name       = "TestAbstractClass$Square"]
    ...
    #[inner_classes     = "TestAbstractClass$Shape:TestAbstractClass:Shape:1032;TestAbstractClass$Square:TestAbstractClass:Square:8"]

    // ── 宏展开输入 ──────────────────────────────────────────────
    #[is_interface      = false]
    #[superclass        = "TestAbstractClass_Shape"]
    #[superclass_fields(color: String)]
    #[all_supertypes    = "TestAbstractClass$Shape;TestAbstractClass$Square;java/lang/Object"]

    pub struct TestAbstractClass_Square {
        #[cfg_attr(any(), java_field(name = "side", descriptor = "D", is_static = false))]
        pub side: f64,
    }

    impl TestAbstractClass_Square {
        ...
        pub fn area(&self) -> Result<f64> {
            let this = self;
            Ok((this.__get_side()*this.__get_side()))
        }
    }
}
