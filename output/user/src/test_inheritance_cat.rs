...
impl From<TestInheritance_Cat> for TestInheritance_Animal {
    fn from(v: TestInheritance_Cat) -> TestInheritance_Animal { v.__into_super() }
}

java_rta_macros::java_class! {
    // ── 字节码元数据 ──────────────────────────────────────────────
    #[binary_name       = "TestInheritance$Cat"]
    ...
    #[inner_classes     = "TestInheritance$Animal:TestInheritance:Animal:8;TestInheritance$Cat:TestInheritance:Cat:8;java/lang/invoke/MethodHandles$Lookup:java/lang/invoke/MethodHandles:Lookup:25"]

    // ── 宏展开输入 ──────────────────────────────────────────────
    #[is_interface      = false]
    #[superclass        = "TestInheritance_Animal"]
    #[superclass_fields(name: String)]
    #[all_supertypes    = "TestInheritance$Animal;TestInheritance$Cat;java/lang/Object"]

    pub struct TestInheritance_Cat;

    impl TestInheritance_Cat {
        ...
        pub fn speak(&self) -> Result<String> {
            let this = self;
            Ok(String::from_owned(format!("{} meows", this.__get_name())))
        }
    }
}
