...
java_rta_macros::java_class! {
    // ── 字节码元数据 ──────────────────────────────────────────────
    #[binary_name       = "TestInheritance$Animal"]
    ...
    #[inner_classes     = "TestInheritance$Animal:TestInheritance:Animal:8;java/lang/invoke/MethodHandles$Lookup:java/lang/invoke/MethodHandles:Lookup:25"]

    // ── 宏展开输入 ──────────────────────────────────────────────
    #[is_interface      = false]
    #[all_supertypes    = "TestInheritance$Animal;java/lang/Object"]

    pub struct TestInheritance_Animal {
        #[cfg_attr(any(), java_field(name = "name", descriptor = "Ljava/lang/String;", is_static = false))]
        pub name: String,
    }

    impl TestInheritance_Animal {
        ...
        pub fn speak(&self) -> Result<String> {
            let this = self;
            Ok(String::from_owned(format!("{} makes a sound", this.__get_name())))
        }
