...
impl From<TestPatternMatch_Rectangle> for Record {
    fn from(v: TestPatternMatch_Rectangle) -> Record { v.__into_super() }
}

java_rta_macros::java_class! {
    // ── 字节码元数据 ──────────────────────────────────────────────
    #[binary_name       = "TestPatternMatch$Rectangle"]
    ...
    #[inner_classes     = "TestPatternMatch$Rectangle:TestPatternMatch:Rectangle:24;TestPatternMatch$Shape:TestPatternMatch:Shape:1544;java/lang/invoke/MethodHandles$Lookup:java/lang/invoke/MethodHandles:Lookup:25"]

    // ── 宏展开输入 ──────────────────────────────────────────────
    #[is_interface      = false]
    #[superclass        = "Record"]
    #[all_supertypes    = "TestPatternMatch$Rectangle;TestPatternMatch$Shape;java/lang/Object;java/lang/Record"]
    #[has_to_string_method = true]
    #[has_hash_code_method = true]

    pub struct TestPatternMatch_Rectangle {
        #[cfg_attr(any(), java_field(name = "width", descriptor = "D", access = "private", modifiers = "final", is_static = false))]
        pub width: f64,
        #[cfg_attr(any(), java_field(name = "height", descriptor = "D", access = "private", modifiers = "final", is_static = false))]
        pub height: f64,
    }

    impl TestPatternMatch_Rectangle {
        #[java_method(name = "<init>", descriptor = "(DD)V", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false, method_parameters = "width:0;height:0")]
        ...
        pub fn toString(&self) -> Result<String> {
                Ok(String::from(format!("Rectangle[width={}, height={}]", self.width.get(), self.height.get()).as_str()))
            }
        ...
        pub fn equals(&self, mut o: Object) -> Result<bool> {
                if let Some(other) = o.0.downcast_ref::<Self>() {
                    Ok(self.__get_width() == other.__get_width() && self.__get_height() == other.__get_height())
                } else {
                    Ok(false)
                }
            }

        #[java_method(name = "width", descriptor = "()D", access = "public", modifiers = "", is_static    = false, is_native    = false, is_abstract  = false, is_synthetic = false)]
