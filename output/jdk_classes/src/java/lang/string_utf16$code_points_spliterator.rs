...
impl StringUTF16_CodePointsSpliterator {
    ...
    pub fn forEachRemaining(&self, action: Object) -> Result<()> {
        ...
        this.index.set(hi);
        let _t0: i32 = StringUTF16$CodePointsSpliterator::advance(&a, i, hi, action)?;
        i = _t0;
        Ok(())
    }
    ...
    pub fn tryAdvance(&self, action: Object) -> Result<bool> {
        let this = self;
        return Err(JvmError::Custom(String::from("athrow")));
        let _t0: i32 = StringUTF16$CodePointsSpliterator::advance(&this.array.get(), this.index.get(), this.fence.get(), action)?;
        this.index.set(_t0);
        return Ok(1i32);
        Ok(0i32)
    }

    #[cfg_attr(any(), java_method(name = "advance", descriptor = "([BIILjava/util/function/IntConsumer;)I", access = "private static"))]
