...
impl<K: Clone + 'static, V: Clone + 'static> ImmutableCollections_MapN<K, V> {
    ...
    pub fn new(input: Vec<Object>) -> Result<Self> {
        let this = Self { table: Field::new(Default::default()), size: Field::new(0), _phantom: std::marker::PhantomData };
        ...
        loop {
            ...
            let _t1: Object = Objects::requireNonNull__obj(input[(i).wrapping_add(1i32) as usize].clone())?;
