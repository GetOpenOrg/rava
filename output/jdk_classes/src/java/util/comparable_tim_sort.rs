...
impl ComparableTimSort {
    ...
    pub fn new(a: Vec<Object>, work: Vec<Object>, workBase: i32, workLen: i32) -> Result<Self> {
        let this = Self { a: Field::new(Default::default()), minGallop: Field::new(0), tmp: Field::new(Default::default()), tmpBase: Field::new(0), tmpLen: Field::new(0), stackSize: Field::new(0), runBase: Field::new(Default::default()), runLen: Field::new(Default::default()) };
        ...
        let mut _arr0: Vec<Object> = Vec::with_capacity(tlen as usize);
        this.tmp.set(_arr0);
        ...
        let mut stackLen: i32 = 49i32;
        let mut _arr1: Vec<i32> = vec![0i32; stackLen as usize];
        this.runBase.set(_arr1);
        let mut _arr2: Vec<i32> = vec![0i32; stackLen as usize];
        this.runLen.set(_arr2);
        Ok(this)
    }

    // java: sort([Ljava/lang/Object;II[Ljava/lang/Object;II)V
    pub fn sort(a: &[Object], lo: i32, hi: i32, work: &[Object], workBase: i32, workLen: i32) -> Result<()> {
        return Err(JvmError::Custom("athrow".to_owned()));
        let mut nRemaining: i32 = (hi).wrapping_sub(lo);
        return Ok(());
        let _t0: i32 = ComparableTimSort::countRunAndMakeAscending(&a, lo, hi)?;
        let mut initRunLen: i32 = _t0;
        ComparableTimSort::binarySort(&a, lo, hi, (lo).wrapping_add(initRunLen))?;
        return Ok(());
        initRunLen = ComparableTimSort::new(a, work, workBase, workLen)?;
        let _t1: i32 = ComparableTimSort::minRunLength(nRemaining)?;
        let mut minRun: i32 = _t1;
        let _t2: i32 = ComparableTimSort::countRunAndMakeAscending(&a, lo, hi)?;
        let mut runLen: i32 = _t2;
        let mut force: i32 = minRun;
        ComparableTimSort::binarySort(&a, lo, (lo).wrapping_add(force), (lo).wrapping_add(runLen))?;
        runLen = force;
    ...
    pub fn binarySort(a: &[Object], lo: i32, hi: i32, start: i32) -> Result<()> {
        return Err(JvmError::Custom("athrow".to_owned()));
        ...
        loop {
            ...
            let _t0 = pivot.compareTo(a[mid as usize].clone())?;
            right = mid;
    ...
    pub fn countRunAndMakeAscending(a: &[Object], lo: i32, hi: i32) -> Result<i32> {
        return Err(JvmError::Custom("athrow".to_owned()));
        let mut runHi: i32 = (lo).wrapping_add(1i32);
        return Ok(1i32);
        runHi = runHi.wrapping_add(1i32);
        let _t0 = a[runHi as usize].clone().compareTo(a[lo as usize].clone())?;
        loop {
            if runHi >= hi { break; }
            let _t0 = a[runHi as usize].clone().compareTo(a[(runHi).wrapping_sub(1i32) as usize].clone())?;
            runHi = runHi.wrapping_add(1i32);
        }
    ...
    pub fn mergeAt(&self, i: i32) -> Result<()> {
        ...
        let _t0: i32 = ComparableTimSort::gallopRight(this.a.get()[base2 as usize].clone(), &this.a.get(), base1, len1, 0i32)?;
        let mut k: i32 = _t0;
        return Err(JvmError::Custom("athrow".to_owned()));
        base1 = (base1).wrapping_add(k);
        len1 = (len1).wrapping_sub(k);
        return Ok(());
        let _t1: i32 = ComparableTimSort::gallopLeft(this.a.get()[((base1).wrapping_add(len1)).wrapping_sub(1i32) as usize].clone(), &this.a.get(), base2, len2, (len2).wrapping_sub(1i32))?;
        len2 = _t1;
        return Err(JvmError::Custom("athrow".to_owned()));
        return Ok(());
    ...
    pub fn gallopLeft(key: Object, a: &[Object], base: i32, len: i32, hint: i32) -> Result<i32> {
        return Err(JvmError::Custom("athrow".to_owned()));
        let mut lastOfs: i32 = 0i32;
        let mut ofs: i32 = 1i32;
        let _t0 = key.compareTo(a[(base).wrapping_add(hint) as usize].clone())?;
        let mut maxOfs: i32 = (len).wrapping_sub(hint);
        loop {
            if ofs >= maxOfs { break; }
            let _t0 = key.compareTo(a[((base).wrapping_add(hint)).wrapping_add(ofs) as usize].clone())?;
            lastOfs = ofs;
        ...
        loop {
            if ofs >= maxOfs { break; }
            let _t0 = key.compareTo(a[((base).wrapping_add(hint)).wrapping_sub(ofs) as usize].clone())?;
            lastOfs = ofs;
            ofs = ((ofs<<(1i32&0x1f))).wrapping_add(1i32);
            ofs = maxOfs;
        }
        ...
        loop {
            ...
            let _t0 = key.compareTo(a[(base).wrapping_add(maxOfs) as usize].clone())?;
    ...
    pub fn gallopRight(key: Object, a: &[Object], base: i32, len: i32, hint: i32) -> Result<i32> {
        return Err(JvmError::Custom("athrow".to_owned()));
        let mut ofs: i32 = 1i32;
        let mut lastOfs: i32 = 0i32;
        let _t0 = key.compareTo(a[(base).wrapping_add(hint) as usize].clone())?;
    ...
    pub fn mergeLo(&self, base1: i32, len1: i32, base2: i32, len2: i32) -> Result<()> {
        ...
        return Err(JvmError::Custom("athrow".to_owned()));
        let _t1 = a[cursor2 as usize].clone().compareTo(tmp[cursor1 as usize].clone())?;
        dest = dest.wrapping_add(1i32);
        cursor2 = cursor2.wrapping_add(1i32);
        a[dest as usize] = a[cursor2 as usize].clone();
        ...
        return Err(JvmError::Custom("athrow".to_owned()));
        let _t2: i32 = ComparableTimSort::gallopRight(a[cursor2 as usize].clone(), &tmp, cursor1, len1, 0i32)?;
        count1 = _t2;
        ...
        a[dest as usize] = a[cursor2 as usize].clone();
        len2 = len2.wrapping_sub(1i32);
        let _t3: i32 = ComparableTimSort::gallopLeft(tmp[cursor1 as usize].clone(), &a, cursor2, len2, 0i32)?;
        count2 = _t3;
    ...
    pub fn mergeHi(&self, base1: i32, len1: i32, base2: i32, len2: i32) -> Result<()> {
        ...
        let mut count2: i32 = 0i32;
        return Err(JvmError::Custom("athrow".to_owned()));
        let _t1 = tmp[cursor2 as usize].clone().compareTo(a[cursor1 as usize].clone())?;
        dest = dest.wrapping_sub(1i32);
        cursor1 = cursor1.wrapping_sub(1i32);
        a[dest as usize] = a[cursor1 as usize].clone();
        count1 = count1.wrapping_add(1i32);
        ...
        len2 = len2.wrapping_sub(1i32);
        return Err(JvmError::Custom("athrow".to_owned()));
        let _t2: i32 = ComparableTimSort::gallopRight(tmp[cursor2 as usize].clone(), &a, base1, len1, (len1).wrapping_sub(1i32))?;
        ...
        a[dest as usize] = tmp[cursor2 as usize].clone();
        len2 = len2.wrapping_sub(1i32);
        let _t3: i32 = ComparableTimSort::gallopLeft(a[cursor1 as usize].clone(), &tmp, tmpBase, len2, (len2).wrapping_sub(1i32))?;
    ...
    pub fn ensureCapacity(&self, minCapacity: i32) -> Result<Vec<Object>> {
        ...
        newSize = _t1;
        let mut _arr2: Vec<Object> = Vec::with_capacity(newSize as usize);
        let mut newArray: Vec<Object> = _arr2;
