...
def _parse_class(data: bytes):
    ...
    解析 .class 文件，返回 (class_name, pool, methods, super_name, iface_names, field_type_refs)
      methods        : list of (name, descriptor, bytecode | None, is_native, catch_types)
      super_name     : 父类 binary name（java/lang/Object 返回 None）
      iface_names    : 实现的接口列表
      field_type_refs: 字段声明描述符中引用的类名列表
    """
    r = _R(data)
    if r.u4() != 0xCAFEBABE:
    ...
    for _ in range(r.u2()):
        ...
        for _ in range(r.u2()):
            ...
            if aname == 'Code':
                ...
                r.skip(alen)
        methods.append((mname, mdesc, bytecode, is_native, catch_types))
...
def _compute_return_summaries(visited: set, cache: dict) -> dict:
    ...
    for cls, name, desc in visited:
        ...
        for mname, mdesc, bc, _, _ in methods:
...
def analyze_cutoff_candidates(
    ...
    for cls in sorted(all_visited_cls):
        ...
        parsed = cache.get(cls)
        if parsed:
            _, _, methods, _, _, _ = parsed
            for mname, mdesc, _, is_native, _ in methods:
                total_methods += 1
        ...
        is_natural_leaf = bool(called) and all(
            any(mname == nm and is_native
                for mname, _, _, is_native, _ in (parsed[2] if parsed else []))
            for nm, _ in called
...
def bfs(user_class_files: list[str], resolver: JdkResolver,
    ...
    for path in user_class_files:
        ...
        for mname, mdesc, bc, _, catch_types in methods:
            # 方法描述符里的参数/返回类型
            for dcls in _extract_desc_classes(mdesc):
                if _in_scope(dcls): refs.add(dcls)
            # 异常表捕获类型
            for dcls in catch_types:
                if _in_scope(dcls): refs.add(dcls)
            if bc:
    ...
    while queue:
        ...
        # JDK 类的字段声明类型（首次解析该类时纳入）
        for dcls in field_refs:
            if _in_scope(dcls): refs.add(dcls)
        for mname, mdesc, bc, is_native, catch_types in methods:
            if mname == name and mdesc == desc:
                ...
                if is_native:
                    ...
                    process(bc, pool)
            # clinit：只要类被实例化（在 instantiated 中），就展开静态初始化块
            elif mname == '<clinit>' and cls in instantiated and bc:
                key_cl = (cls, '<clinit>', mdesc)
                if key_cl not in visited:
                    visited.add(key_cl)
                    if call_graph is not None:
                        _cur[0] = key_cl
                    process(bc, pool)
...
def main():
    ...
    vm, vcls, vnat = r2_m, r2_cls, r2_nat
    ...
    print('╠═══════════════════════════════════════════════════════════════════════╣')
    for rname, ms, cls, nat, cc, ro in rows:
        print(f'║  {rname:<22}  │  {len(cc):>6}  │  {len(ro):>6}  │  {len(cls):>5}  │  {len(ms):>5} │  {len(nat):>4} ║')
    print('╚═══════════════════════════════════════════════════════════════════════╝')
    ...
    # 这些类出现在 ref_only_cls 里但不在调用链方法里
    ...
    with open(report, 'w', encoding='utf-8') as f:
        ...
        f.write('|------|--------:|------:|-----:|-------:|-------:|\n')
        for rname, ms, cls, nat, cc, ro in rows:
            f.write(f'| {rname} | {len(cc)} | {len(ro)} | {len(cls)} | {len(ms)} | {len(nat)} |\n')
        f.write('\n')
        ...
        for cls2, nm2, desc2 in r5_m: by5.setdefault(cls2, []).append(f'{nm2}{desc2}')
