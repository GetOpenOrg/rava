...
# build.rs 模板：路径相对于 jdk_classes/ crate 根，native_impls 在 workspace 根
BUILD_RS = """\
...
def write_cargo_project(out_dir: str, class_infos: list[ClassInfo],
    ...
    # build.rs：从模板生成，native_impls 在 workspace 根（../native_impls）
    _write(os.path.join(jdk_dir, 'build.rs'), BUILD_RS)
