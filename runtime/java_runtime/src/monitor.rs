...
//! ## 遗留（随线程模型一并收敛）
//!
//!   - `wait` 的 `InterruptedException`（中断唤醒）：线程未真实运行（`Thread.start0`
//!     未实现），构造该异常会引入非全量闭包类，随 threading 模型落地。
...
impl Monitor {
    ...
    /// 参数异常先于持有检查）→ 释放全部重入计数 → 在等待队列上阻塞至
    /// notify / 超时 / 虚假唤醒 → 按原计数重新获取监视器。
    /// `millis == 0 && nanos == 0` 为无限等待。调用方以条件循环消费虚假唤醒。
